...
int clone(void (*fcn)(void *), void *arg, void *stack)
{
  ...
  // Set trap frames
  *np->tf = *this_proc->tf;
  // Initialize stack and base resgisters
  uint thread_stack_pointer = (uint)stack + PGSIZE;

  // Move user args to stack before call user function
  int regs[2];
  regs[0] = 0xffffffff;
...
int join(void **stack)
{
  // This is used for checking the ptable
  struct proc *p;
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state != ZOMBIE)
        continue;
      if(p->state == ZOMBIE){
        // Found one zombie children.
        // dec the number of threads for the process
        *stack = p->stack;
        // fetch the pid of the child
        pid = p->pid;
        // freeing kernel stack
        kfree(p->kstack);
        p->kstack = 0;
        //freevm(p->pgdir);
        // Changing child state to unused
        p->state = UNUSED;
        // removing pid of the process
        p->pid = 0;
        // removing the parent of the process
        p->parent = 0;
        p->name[0] = 0;
        // killing the child process
        p->killed = 0;
        // releasing ptable lock
        release(&ptable.lock);
        // returning child process id
        return pid;
      }
    }
